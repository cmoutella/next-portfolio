  ...
  return (
    <section className={`w-full ${classes.mainColor}`}>
      ...
      >
        ...
        >
          <h1
            className="uppercase text-center text-white text-[60px] font-paytone tracking-wider /// relative"
            style={{ zIndex: 2 }}
          >
            {title}
            <span
              className="text-primary-medium absolute right-0 top-0 translate-y-[-8px] translate-x-[-8px]"
              style={{ zIndex: -1 }}
            >
              {effectWord}
            </span>
          ...
          {subtitle && (
            <h3 className="text-white text-[18px] tracking-wider block mt-[20px]">
              {subtitle}
            </h3>
          )}
        </div>
